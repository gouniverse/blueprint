...
func (controller *blogPostManagerController) prepareData(r *http.Request) (data blogPostManagerControllerData, errorMessage string) {
	...
	userPage := helpers.UserSettingGet(r, "blog_page", "")
	userPerPage := helpers.UserSettingGet(r, "blog_per_page", strconv.Itoa(10))
	userSortOrder := helpers.UserSettingGet(r, "blog_sort_order", sb.DESC)
	userSortBy := helpers.UserSettingGet(r, "blog_sort_by", blogstore.COLUMN_CREATED_AT)
	userSearch := helpers.UserSettingGet(r, "blog_filter_search", "")
	userDateFrom := helpers.UserSettingGet(r, "blog_filter_date_from", carbon.Now().AddDays(-31).ToDateString())
	userDateTo := helpers.UserSettingGet(r, "blog_filter_date_to", carbon.Now().ToDateString())

	data.page = userPage
	data.pageInt = cast.ToInt(userPage)
	data.perPage = cast.ToInt(userPerPage)
	data.sortOrder = utils.Req(r, "sort", userSortOrder)
	data.sortBy = utils.Req(r, "by", userSortBy)
	data.status = utils.Req(r, "status", "")
	data.search = utils.Req(r, "search", userSearch)
	data.dateFrom = utils.Req(r, "date_from", userDateFrom)
	data.dateTo = utils.Req(r, "date_to", userDateTo)
	data.customerID = utils.Req(r, "customer_id", "")

	helpers.UserSettingSet(r, "blog_page", data.page)
	helpers.UserSettingSet(r, "blog_per_page", cast.ToString(data.perPage))
	helpers.UserSettingSet(r, "blog_sort_by", data.sortBy)
	helpers.UserSettingSet(r, "blog_sort_order", data.sortOrder)
	helpers.UserSettingSet(r, "blog_filter_search", data.search)
	helpers.UserSettingSet(r, "blog_filter_date_from", data.dateFrom)
	helpers.UserSettingSet(r, "blog_filter_date_to", data.dateTo)
	...
	data.blogList, err = config.BlogStore.EnableDebug(true).PostList(query)
