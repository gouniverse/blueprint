...
func (controller *queueManagerController) modalQueuedTaskDelete(_ *http.Request, queueID string) *hb.Tag {
	...
	isMultiple := strings.Contains(queueID, ",")
	...
	title := hb.Heading5().
		TextIfElse(isMultiple, "Delete Selected Tasks", "Delete Queued Task").
	...
	groupDetails := hb.Div().
		Class("text-danger").
		TextIfElse(isMultiple, "Are you sure you want to delete the selected tasks?", "Are you sure you want to delete this queued task?").
		Child(hb.BR()).
